...
TEST_CASE("Version numbers are correctly set and returned.", "[common]") {
    SECTION("Public API calls.") {
        REQUIRE( idi_common_get_version_major() == IDI_VERSION_MAJOR );
        REQUIRE( idi_common_get_version_minor() == IDI_VERSION_MINOR );
        REQUIRE( idi_common_get_version_patch() == IDI_VERSION_PATCH );
    }
}